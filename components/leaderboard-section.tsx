...
import type { EditorLeaderboardEntry, LeaderboardEntry } from '@/server/data/leaderboard'
...
const MAX_EDITOR_CHART_ITEMS = 5

export function EditorLeaderboardChart({
  title,
  description,
  entries,
}: {
  title: string
  description: string
  entries: EditorLeaderboardEntry[]
}) {
  const chartData = entries.slice(0, MAX_EDITOR_CHART_ITEMS).map((entry, index) => ({
    editorKey: `editor-${entry.editor}`,
    label: entry.label,
    votes: entry.votes,
  ...
  return (
    <Card className="bg-card/80 backdrop-blur-sm">
      ...
      <CardContent>
        {chartData.length === 0 ? (
          <p className="text-muted-foreground text-sm">No editor selections yet.</p>
        ) : (
          <div className="space-y-3">
            <ChartContainer config={chartConfig} className="aspect-auto h-44 w-full">
              <BarChart accessibilityLayer data={chartData} margin={{ top: 8, right: 8, left: 8 }}>
                ...
                <Bar dataKey="votes" fill="var(--chart-1)" radius={8}>
                  {chartData.map((item) => (
                    <Cell key={item.editorKey} fill={item.fill} />
                  ))}
                </Bar>
              </BarChart>
            </ChartContainer>
            ...
            >
              {chartData.map((item) => (
                <li key={item.editorKey} className="min-w-0 px-1 wrap-break-word" title={item.label}>
                  {item.label}
                </li>
              ))}
            </ul>
